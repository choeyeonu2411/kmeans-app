...
def main() :
    ...
    if file is not None :
        ...
        st.info('K-means를 수행하기 위한 프레임입니다.')
        st.dataframe(df_new)

        st.subheader('최적의k값을 찾기 위해 wcss를 구합니다.')
        ...
        if df_new.shape[0]<10 :
            max_k=st.slider('k값 선택(최대그룹갯수)',min_value=2,max_value=[0])
        else :
            max_k=st.slider('k값 선택(최대그룹갯수)',min_value=2,max_value=10)

        is_complete_wcss=False
